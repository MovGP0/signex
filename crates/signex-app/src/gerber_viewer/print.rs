    ...
    let (page_width_mm, page_height_mm) = match layout.page_size
    {
        GerberPageSize::FullSize => (
            layout
                .bounds
                .width()
                .max(MIN_FULL_SIZE_DIMENSION_MM)
                + PRINT_MARGIN_MM * 2.0,
            layout
                .bounds
                .height()
...
fn render_content(plan: &GerberPrintPlan) -> String
{
    ...
    let transform = |point: Point| Point {
        x: offset_x
            + (point.x - plan.artwork_bounds_mm.min.x) * plan.content_scale,
        y: offset_y
            + (point.y - plan.artwork_bounds_mm.min.y) * plan.content_scale,
    };
