...
impl Signex {
    ...
    pub(crate) const fn handle_bom_preview_set_format(&mut self, format: BomFormat) {
        if let Some(preview) = self.document_state.bom_preview.as_mut() {
    ...
    /// the rollup default.
    pub(crate) const fn handle_bom_preview_sort_column(&mut self, idx: usize) {
        if let Some(preview) = self.document_state.bom_preview.as_mut() {
    ...
    pub(crate) const fn handle_bom_preview_column_drag_start(&mut self, idx: usize) {
    ...
    pub(crate) fn handle_bom_preview_export(&mut self) -> Option<Task<Message>> {
        ...
        let filter_exts_owned: Vec<String> = filter_exts
            .iter()
    ...
    pub(crate) fn handle_export_bom_finished(
        ...
        let opts = if let Some(mut user_opts) = self.document_state.pending_bom_options.take() {
