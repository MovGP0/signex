        ...
        for command in &layer.commands
        {
            ...
            match command
            {
                ...
                ExcellonCommand::Coordinate(x, y, format) => {
                    current = resolve_excellon_point(
                        *x,
                        *y,
                        current,
                        incremental,
                ...
                ExcellonCommand::Slot {
                    ...
                    let start = resolve_excellon_point(
                        *from_x,
                        *from_y,
                        current,
                        incremental,
                    ...
                    let end = resolve_excellon_point(
                        *to_x,
                        *to_y,
                        start,
                        incremental,
...
fn resolve_coordinates(coordinates: Option<&Coordinates>, current: Point) -> Point
{
    ...
    Point {
        x: resolve_axis(coordinates.x.map(f64::from), current.x, coordinates.format.coordinate_mode),
        y: resolve_axis(coordinates.y.map(f64::from), current.y, coordinates.format.coordinate_mode),
    }
}
