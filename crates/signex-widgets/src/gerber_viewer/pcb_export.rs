...
use signex_types::pcb::{
    BoardGraphic, Footprint, LayerDef, Pad, PadShape, PadType, PcbBoard,
    PcbSetup, Point as PcbPoint, Segment, PCB_DEFAULT_CLEARANCE_MM,
    PCB_DEFAULT_THICKNESS_MM, PCB_DEFAULT_TRACE_WIDTH_MM,
    PCB_DEFAULT_VIA_DIAMETER_MM, PCB_DEFAULT_VIA_DRILL_MM, PCB_GRID_MM,
...
)
{
    match primitive
    {
        ...
        } if valid_point(*position) =>
        {
            ...
            {
                report.skip(format!(
                    "{source_layer}: flashes on non-copper layers",
                ));
                return;
            };
            ...
            {
                report.skip(format!(
                    "{source_layer}: polygon or macro flash",
                ));
                return;
            };
            pads_by_layer
...
fn pad_geometry(aperture: &ApertureShape) -> Option<(PadShape, PcbPoint)>
{
    match aperture
    {
        ApertureShape::Circle { diameter } if valid_size(*diameter) =>
        {
            Some((
                PadShape::Circle,
                PcbPoint::new(*diameter, *diameter),
            ))
        }
        ApertureShape::Rectangle { width, height }
            if valid_size(*width) && valid_size(*height) =>
        {
...
fn target_layer(layer_type: LayerType) -> Option<TargetLayer>
{
    match layer_type
    {
        ...
        LayerType::Inner(index) if index > 0 =>
        {
            Some(TargetLayer::Copper(format!("In{index}.Cu")))
        }
        LayerType::PasteTop => Some(TargetLayer::Graphic("F.Paste".to_owned())),
        LayerType::PasteBottom =>
        {
            Some(TargetLayer::Graphic("B.Paste".to_owned()))
        }
        ...
        {
            Some(TargetLayer::Graphic("F.SilkS".to_owned()))
        }
        LayerType::SilkScreenBottom =>
        {
            Some(TargetLayer::Graphic("B.SilkS".to_owned()))
        }
