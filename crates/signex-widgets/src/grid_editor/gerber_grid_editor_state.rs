...
use crate::gerber_viewer::{
    DEFAULT_GRID_INDEX, GerberViewerState, GridSizeChoice, GridSizePreset,
    GridUnit, default_grid_catalog, grid_size_choices,
    persist_grid_catalog,
};
...
impl GerberGridEditorState
{
    ...
    ) -> GerberGridEditorOutcome
    {
        match message
        {
            ...
            GerberGridEditorMessage::ResetDefaults =>
            {
                ...
                self.selected_index =
                    DEFAULT_GRID_INDEX.min(self.catalog.len() - 1);
                self.load_selected_settings();
            }
    ...
    pub fn apply_to(
        &self,
        viewer: &mut GerberViewerState,
    ) -> Result<(), String>
    {
        persist_grid_catalog(&self.catalog)?;
        let changed = viewer.grid_catalog != self.catalog
            || viewer.active_grid_index != self.selected_index;
        viewer.grid_catalog = self.catalog.clone();
        viewer.active_grid_index = self
            .selected_index
            .min(viewer.grid_catalog.len() - 1);
        if changed
        {
            viewer.redraw_generation =
                viewer.redraw_generation.wrapping_add(1);
        }
