...
fn write_child_sheet(out: &mut String, cs: &ChildSheet) {
    ...
    // "Sheetname" (one word, no space) is the legacy Standard 5/6 token that most
    // Standard versions accept for backward compatibility.  The parser reads both
    // this form and the Standard 7+ "Sheet name" (two words) form; we intentionally
